...
const stack = [
  ...
  { name: "GitHub", icon: "devicon-github-original text-white" }, // ✨ Corrección: Forzado a blanco para fondo negro
...
export default function Stack() {
  // Estado para pausar las órbitas cuando el usuario inspecciona un elemento
  const [isPaused, setIsPaused] = useState(false);

  // Animaciones base dinámicas controladas por el estado "isPaused"
  ...
  return (
    <section className="relative py-40 bg-black select-none overflow-hidden">
      <Container>
        ...
        {/* Orbit Container */}
        <div
          className="relative flex items-center justify-center h-[820px] w-full scale-90 md:scale-100"
        ...
        >
          ...
          {/* Center Circle */}
          <div className="absolute z-20 w-40 h-40 rounded-full border border-white/10 bg-zinc-900/50 backdrop-blur-xl flex items-center justify-center shadow-[0_0_60px_rgba(59,130,246,0.15)]">
          ...
          >
            {stack.map((tech, index) => {
              ...
              return (
                /* Contenedor estructural del ángulo */
                <div
                  key={tech.name}
                ...
                >
                  {/* Nodo animado aislado de los estilos de posición */}
                  <motion.div
                    variants={counterOrbitAnimation}
                    animate={isPaused ? "paused" : "animate"}
                    className="flex flex-col items-center justify-center w-20 h-20 rounded-2xl border border-white/10 bg-zinc-900/40 backdrop-blur-xl shadow-lg hover:scale-110 hover:bg-white/10 hover:border-white/20 transition-all duration-300 cursor-pointer"
                  >
                    <i
                      className={`${tech.icon} text-3xl`}
                      aria-label={tech.name}
                    ></i>
                    <span className="mt-1.5 text-[11px] text-zinc-300 font-medium">
                      {tech.name}
                    </span>
                  </motion.div>
                </div>
              ...
              return (
                ...
                >
                  ...
                  <motion.div
                    ...
                    animate={isPaused ? "paused" : "animate"}
                    className="flex flex-col items-center justify-center w-28 h-24 rounded-2xl border border-blue-500/20 bg-blue-950/20 backdrop-blur-xl shadow-lg hover:scale-110 hover:border-blue-400/40 transition-all duration-300 group cursor-pointer"
                  >
                    ...
                    <span className="text-[9px] text-zinc-400 text-center px-1 font-medium✨">
                      {tool.role}
                    </span>
                  </motion.div>
                </div>
